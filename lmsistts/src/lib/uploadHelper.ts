...
// Fungsi untuk upload Thumbnail Kursus
export async function uploadCourseThumbnail(
  file: File,
  title: string,
  courseId: number
  ...
  try {
    ...
    return {
      success: true,
      url: blob.url, // URL dari Blob (https://...)
    };
  } catch (error: any) {
    console.error("Upload Thumbnail Error:", error);
    return { success: false, error: "Gagal menyimpan thumbnail ke Blob." };
  }
}

// Fungsi Umum (PDF, Tugas, dan Profile Image)
export async function uploadToPublic(file: File, subfolder: string) {
  try {
    // Buat nama file unik
    ...
    return {
      success: true,
      url: blob.url, // URL dari Blob
    };
  } catch (error: any) {
    console.error(`Upload ${subfolder} Error:`, error);
    return { success: false, error: `Gagal upload ke Blob (${subfolder}).` };
  }
}

// Fungsi khusus untuk Profile Image (jika action Anda memanggil fungsi bernama uploadImage)
// Tambahkan ini untuk jaga-jaga jika action Anda memanggil nama fungsi yang berbeda
