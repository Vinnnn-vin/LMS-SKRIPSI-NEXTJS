    ...
    startTransition(async () => {
      ...
      if (result.success) {
        ...
        if (result.certificateGranted) {
          notifications.show({
            title: "SELAMAT!",
            message: "Anda telah menyelesaikan kursus!",
            color: "teal",
            autoClose: 10000,
          });
        }
...
function AssignmentSubmission({
  ...
  const handleSubmit = () => {
    if (!file && !text) {
      notifications.show({
        title: "Error",
        message: "Pilih file atau isi jawaban teks untuk dikumpulkan.",
        color: "red",
      });
      return;
    }
    ...
    startTransition(async () => {
      try {
        ...
        // Kalau pakai file, upload dulu ke storage kamu (misal S3, supabase, dsb.)
        if (file) {
          // TODO: ganti bagian ini dengan fungsi upload ke storage kamu
          // Misalnya:
          // fileUrl = await uploadFileToStorage(file);
          fileUrl = `/uploads/${file.name}`; // placeholder
        } else {
          // Simpan teks sebagai file teks sementara
          const blob = new Blob([text], { type: "text/plain" });
          // TODO: upload blob ke storage
          fileUrl = `/uploads/jawaban-${Date.now()}.txt`;
        }

        const result = await createAssignmentSubmission(
          detail.material_detail_id, // assignmentId
          courseId, // courseId
          fileUrl // fileUrl
        );
    ...
    <Paper p="md" withBorder radius="md">
      ...
      {isCompleted ? (
        ...
        <Stack mt="md">
          <FileInput
            label="Upload File"
            placeholder="Pilih file tugas Anda (.pdf, .docx, .zip)"
            onChange={setFile}
            value={file}
          />
          <Textarea
            label="Atau Tulis Jawaban"
...
function QuizPlayer({
  ...
  const handleSubmitQuiz = () => {
    startTransition(async () => {
      ...
      notifications.show({
        title: "Simulasi",
        message: "Quiz berhasil dikumpulkan (simulasi).",
        color: "green",
      });
  ...
  return (
    <Stack>
      <Alert
        color="orange"
        title="Pengerjaan Quiz (UI Sederhana)"
        icon={<IconQuestionMark />}
      >
        Fitur pengerjaan quiz lengkap dengan timer dan validasi jawaban perlu
              ...
              <Stack gap="xs">
                {q.options?.map((opt: any) => (
                  <Button
                    key={opt.option_id}
                    variant={
                      answers[q.question_id] === String(opt.option_id)
                        ? "filled"
                        : "outline"
                    }
                    onClick={() =>
              ...
              <Stack gap="xs">
                {q.options?.map((opt: any) => (
                  <Button
                    key={opt.option_id}
                    variant={
                      (answers[q.question_id] as string[])?.includes(
                        String(opt.option_id)
                      )
                        ? "filled"
                        : "outline"
...
const getMaterialIcon = (type: number) => {
  switch (type) {
    ...
    case 4:
      return <IconPencil size={16} />; // Tugas
    default:
      return null;
  }
};
...
export function CourseLearningClientUI({
  ...
  course: any;
  completedItems: { details: Set<number>; quizzes: Set<number> };
  enrollmentId: number;
  totalProgress: number;
}) {
        ...
        <Stack h="100%" justify="space-between">
          <Box>
            ...
            {detail.material_detail_type === 1 && (
              <Alert color="blue" title="Konten Video">
                (Placeholder Pemutar Video untuk: {detail.materi_detail_url})
              </Alert>
            )}
            ...
            {detail.material_detail_type === 3 &&
              detail.materi_detail_url.includes("youtube.com") && (
                <Box
                  style={{
                    position: "relative",
                    paddingBottom: "56.25%",
                ...
                >
                  <iframe
                    ...
                    src={`https://www.youtube.com/embed/${new URL(detail.materi_detail_url).searchParams.get("v")}`}
                    title={detail.material_detail_name}
                    frameBorder="0"
                    allowFullScreen
                  ></iframe>
                  ...
                  <Accordion.Panel>
                    <Stack gap="xs">
                      ...
                      {material.details?.map((detail: any) => {
                        const isCompleted = completedDetails.has(
