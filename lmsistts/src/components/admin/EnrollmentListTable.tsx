...
import { Box, Group, TextInput, Text, Tooltip, Progress, Badge, Anchor } from '@mantine/core';
import { IconSearch } from '@tabler/icons-react';
import sortBy from 'lodash/sortBy';
import dayjs from 'dayjs'; // Anda mungkin perlu: npm install dayjs
...
export function EnrollmentListTable({ enrollments }: { enrollments: EnrollmentRow[] }) {
    const [page, setPage] = useState(1);
    const [sortStatus, setSortStatus] = useState<DataTableSortStatus<EnrollmentRow>>({ columnAccessor: 'enrolled_at', direction: 'desc' });
    const [query, setQuery] = useState('');
    const [records, setRecords] = useState<EnrollmentRow[]>([]);
    
    // Gunakan useMemo untuk performa filter/sort
    const filteredAndSortedRecords = useMemo(() => {
        let data = [...enrollments];
        if (query) {
            const lowerQuery = query.toLowerCase();
            data = data.filter((e) =>
                `${e.student.first_name || ''} ${e.student.last_name || ''}`.toLowerCase().includes(lowerQuery) ||
    ...
    return (
        <Box>
            ...
            <DataTable<EnrollmentRow>
                ...
                withTableBorder borderRadius="sm" highlightOnHover
                records={records}
                columns={[
                    { 
                    ...
                    { 
                        ...
                        title: 'Email', 
                        sortable: true,
                        render: ({ student }) => <Anchor href={`mailto:${student.email}`}>{student.email}</Anchor>
                    },
                    {
                        accessor: 'progress',
                        title: 'Progres',
                    ...
                    },
                    {
                        ...
                        sortable: true,
                        render: ({ enrolled_at }) => enrolled_at ? dayjs(enrolled_at).format('DD MMM YYYY') : '-',
                    },
                    {
                        accessor: 'access_expires_at',
                        title: 'Akses Berakhir',
                        sortable: true,
                        render: ({ access_expires_at }) => formatExpiryDate(access_expires_at),
                    },
                ]}
                sortStatus={sortStatus}
                onSortStatusChange={setSortStatus}
                totalRecords={filteredAndSortedRecords.length} // Total setelah filter
