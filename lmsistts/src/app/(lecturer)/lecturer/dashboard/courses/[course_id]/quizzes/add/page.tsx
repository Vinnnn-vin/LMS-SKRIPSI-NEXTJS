...
export default function AddQuizPage({
  ...
  const handleSubmit = (values: CreateQuizInput) => {
    ...
    startTransition(async () => {
      ...
      if (result.error) {
        ...
        router.push(
          `/lecturer/dashboard/courses/${courseId}/quizzes/${result.data.quiz_id}/edit`
        );
      }
    });
  };
  ...
  if (isNaN(materialId)) {
    return (
      <Alert color="red">ID Materi tidak ditemukan. Silakan kembali.</Alert>
    );
  }
    ...
    <Container size="md" py="xl">
      ...
      <Paper withBorder shadow="md" p={30} radius="md" pos="relative">
        ...
        <form onSubmit={form.onSubmit(handleSubmit)}>
          <Stack>
            {error && (
              <Alert title="Gagal" color="red" icon={<IconAlertCircle />}>
                {error}
              </Alert>
            )}
            <TextInput
            ...
            <Textarea
              ...
              placeholder="Instruksi singkat untuk quiz..."
              minRows={3}
              {...form.getInputProps("quiz_description")}
            />
            <Group grow>
              <NumberInput
                label="Skor Lulus (0-100)"
                min={0}
                max={100}
                required
