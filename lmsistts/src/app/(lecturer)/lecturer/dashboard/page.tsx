...
export default async function LecturerDashboardPage() {
    ...
    const stats = [
        { title: 'Total Kursus Anda', value: formatNumber(statsResult.data.totalCourses), icon: IconBook, color: 'violet' },
        { title: 'Total Siswa Anda', value: formatNumber(statsResult.data.totalStudents), icon: IconUsers, color: 'blue' },
        { title: 'Total Pendapatan', value: formatRupiah(statsResult.data.totalSales), icon: IconCash, color: 'green' },
    ];

    return (
        <Container fluid>
            <Stack>
                <Title order={3}>Selamat Datang, {session?.user?.name || 'Dosen'}</Title>
                <Text c="dimmed">Berikut adalah ringkasan aktivitas mengajar Anda di platform iClick.</Text>
            </Stack>
