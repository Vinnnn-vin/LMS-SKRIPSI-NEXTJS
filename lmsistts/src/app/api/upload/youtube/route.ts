  ...
  try {
    ...
    const response = await youtube.videos.insert({
      ...
      requestBody: {
        ...
        status: {
          privacyStatus: isFree ? 'public' : 'private',
    ...
    if (error.code === 403) {
      if (error.message?.includes('quota')) {
        return NextResponse.json(
          { success: false, error: 'Kuota upload YouTube habis. Coba lagi besok.' },
          { status: 429 }
        );
      }
      return NextResponse.json(
  ...
  if (code) {
    try {
      ...
      return NextResponse.json({
        ...
        message: '✅ SIMPAN refresh_token ini ke .env.local sebagai YOUTUBE_REFRESH_TOKEN'
      });
    } catch (error: any) {
