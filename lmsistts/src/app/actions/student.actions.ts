...
export async function getMyEnrolledCoursesWithProgress() {
  try {
    ...
    const quizzesCount = await Material.findAll({
      ...
      include: [
        { model: Quiz, as: "quizzes", attributes: [], required: false },
      ],
      group: ["Material.course_id"],
      raw: true,
    });
...
export async function getCourseLearningData(courseId: number) {
  try {
    ...
    const enrollment = await Enrollment.findOne({
      where: { course_id: courseId, user_id: userId, status: "active" },
    });
    if (!enrollment) {
      throw new Error(
        "Anda tidak terdaftar di kursus ini atau kursus telah selesai."
      );
    }
    ...
    return {
      ...
      data: {
        ...
        completedSet,
        enrollment_id: enrollment.enrollment_id,
        completedItems: { details: completedSet, quizzes: new Set<number>() },
        totalProgress:
          course.materials?.reduce((acc, material) => {
            const detailCount = material.details?.length || 0;
            const completedCount = material.details?.reduce(
              (dAcc, detail) =>
                dAcc + (completedSet.has(detail.material_detail_id) ? 1 : 0),
              0
            ) || 0;
            return acc + (detailCount > 0 ? (completedCount / detailCount) : 0);
          }, 0) /
          (course.materials?.length || 1) * 100,
...
// --- FUNGSI UNTUK MENGIRIMKAN TUGAS ---
export async function createAssignmentSubmission(
  ...
  try {
    ...
    // Simpan submission
    // Asumsikan ada model AssignmentSubmission
    await sequelize.models.AssignmentSubmission.create({
