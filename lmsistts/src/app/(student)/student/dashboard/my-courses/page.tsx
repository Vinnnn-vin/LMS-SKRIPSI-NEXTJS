...
// Komponen Kartu Kursus (Bisa diekstrak)
    ...
    return (
        <Card shadow="sm" padding="lg" radius="md" withBorder>
            ...
            <Stack gap="xs" mt="sm">
                ...
                <Progress value={course.progress} radius="sm" animated={!isCompleted} color={isCompleted ? 'green' : 'blue'} />
            </Stack>
            
            {isCompleted ? (
                <Button
                    component={Link}
                    href="/student/dashboard/my-certificates" // Arahkan ke daftar sertifikat
                    variant="light"
                    color="green"
                    fullWidth
                    mt="md"
...
export default async function MyCoursesPage() {
    ...
    return (
        <Container fluid>
            <Title order={3} mb="lg">Kursus Saya</Title>
            <Tabs defaultValue="active" variant="outline" radius="md">
                <TabsList>
                    <TabsTab value="active">Kursus Aktif ({active.length})</TabsTab>
                    <TabsTab value="completed">Kursus Selesai ({completed.length})</TabsTab>
                </TabsList>

                <TabsPanel value="active" pt="lg">
                    {active.length > 0 ? (
                        <SimpleGrid cols={{ base: 1, md: 2, lg: 3 }}>
                            {active.map((course: any) => <MyCourseCard key={course.enrollment_id} course={course} />)}
                        </SimpleGrid>
                    ) : (
                ...
                <TabsPanel value="completed" pt="lg">
                    {completed.length > 0 ? (
                        <SimpleGrid cols={{ base: 1, md: 2, lg: 3 }}>
                            {completed.map((course: any) => <MyCourseCard key={course.enrollment_id} course={course} />)}
                        </SimpleGrid>
                    ) : (
                        <Text c="dimmed" ta="center" mt="xl">Belum ada kursus yang selesai.</Text>
                    )}
                </TabsPanel>
            </Tabs>
        </Container>
